 ...
 *--------------------------------------------------------------------------*/
    ...
    slI2C_Start();
    slI2C_WriteByte(bme280.dev_addr);
    slI2C_Stop();
...
s8 BME280_I2C_bus_write(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt) {
    ...
    */
    slI2C_Start();
    slI2C_WriteByte(dev_addr);
    slI2C_WriteByte(reg_addr);
    slI2C_Start();
    slI2C_WriteByte(dev_addr);
    for (stringpos = BME280_INIT_VALUE; stringpos < cnt; stringpos++) {
        array[stringpos + BME280_DATA_INDEX] = *(reg_data + stringpos);
...
s32 bme280_data_readout_template(void) {
    ...
    com_rslt += bme280_read_uncomp_pressure_temperature_humidity(&v_data_uncomp_temp_s32, &v_data_uncomp_pres_s32,
    ...
    com_rslt += bme280_read_pressure_temperature_humidity(&v_comp_press_u32[1], &v_comp_temp_s32[1],
